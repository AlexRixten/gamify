...
import { fitnessPreset } from '../fitness';
  ...
  render: () => (
    <View>
      <View style={styles.section}>
        <Text style={styles.sectionTitle}>Levels ({fitnessPreset.levels.length})</Text>
        <View style={styles.levelsContainer}>
          {fitnessPreset.levels.map((level, index) => (
      ...
      <View style={styles.section}>
        <Text style={styles.sectionTitle}>Badges ({fitnessPreset.badges?.length || 0})</Text>
        <View style={styles.badgesGrid}>
          {fitnessPreset.badges?.map((badge) => (
...
export const Achievements: StoryObj = {
  render: () => {
    const categories = [...new Set(fitnessPreset.achievements?.map((a) => a.category))];
    return (
      <View>
        {categories.map((category) => (
          <View key={category} style={styles.section}>
            <Text style={styles.sectionTitle}>
              {category.charAt(0).toUpperCase() + category.slice(1)} ({fitnessPreset.achievements?.filter((a) => a.category === category).length})
            </Text>
            {fitnessPreset.achievements
...
export const Badges: StoryObj = {
  render: () => {
    const badgesByRarity = {
      common: fitnessPreset.badges?.filter((b) => b.rarity === 'common') || [],
      rare: fitnessPreset.badges?.filter((b) => b.rarity === 'rare') || [],
      epic: fitnessPreset.badges?.filter((b) => b.rarity === 'epic') || [],
      legendary: fitnessPreset.badges?.filter((b) => b.rarity === 'legendary') || [],
    };
